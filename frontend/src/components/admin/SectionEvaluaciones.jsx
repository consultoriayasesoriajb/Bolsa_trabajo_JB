...
export default function SectionEvaluaciones() {
  ...
  const renderEstrellas = (n) => (
    <span className={`flex items-center gap-0.5 font-black text-sm ${ESTRELLAS_COLOR[n] || "text-slate-400"}`}>
      {"★".repeat(n)}{"☆".repeat(5 - n)}
    </span>
  );
              ...
              {loading ? (
                ...
                <tr key={ev.id} className="border-t border-slate-50 hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-3 py-2 md:px-5 md:py-3.5">
                    <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-[#123498]/10 text-[#123498] text-xs font-black">
                      {ev.iniciales}
                    </span>
                  </td>
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-[#123498]/10 text-[#123498] text-sm font-black">
                  {modalEval.iniciales}
                </span>
                <span className={`inline-block px-2.5 py-0.5 rounded text-[9px] font-black uppercase tracking-wider ${
              ...
              {modalEval.texto_positivo && (
                <div className="bg-gris-oscuro/10 border-l-4 border-green-500 rounded-xl p-3">
                  <p className="text-xs font-black text-green-600 uppercase tracking-wider mb-1">Lo bueno</p>
                  <p className="text-sm text-slate-600 leading-relaxed">{modalEval.texto_positivo}</p>
                </div>
              )}

              {modalEval.texto_negativo && (
                <div className="bg-gris-oscuro/10 border-l-4 border-red-500 rounded-xl p-3">
                  <p className="text-xs font-black text-red-500 uppercase tracking-wider mb-1">Lo que mejoraría</p>
                  <p className="text-sm text-slate-600 leading-relaxed">{modalEval.texto_negativo}</p>
                </div>
              )}

              <div className="flex flex-row bg-slate-50 rounded-xl p-3">
                <p className="font-black text-sm">Recomendaria la empresa: </p>
                <p className="text-sm text-slate-500 font-semibold text-right ml-auto">
              ...
              <div className="flex gap-2 pt-2 border-t border-slate-100">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border border-naranja text-naranja text-sm font-black uppercase tracking-wider hover:bg-naranja/10 transition-colors"
                >
                  {modalEval.estado === "visible"
                ...
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border border-red text-red-600 text-sm font-black uppercase tracking-wider hover:bg-red-100 transition-colors"
                >
