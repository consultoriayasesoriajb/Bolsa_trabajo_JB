  ...
  return (
    <div className="space-y-6">
      ...
      {modalOpen && (
        ...
        >
          {/* 1. CONTENEDOR PRINCIPAL: Le quitamos el p-6 y el overflow, y lo hacemos flex col */}
          <div
            className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] flex flex-col border-b-4 overflow-hidden"
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="flex flex-col overflow-hidden">
              ...
              {/* 3. ÁREA DE SCROLL: Aquí van todos los inputs */}
              <div className="overflow-y-auto p-6 space-y-6">
                <div className="grid grid-cols-2 gap-6">
                  {/* Nombre */}
                  <div>
                    <label className="text-sm font-black text-naranja/80 mb-1 block">Nombre *</label>
                    <input
                      type="text"
                      value={formNombre}
                  ...
                  {/* RUC */}
                  <div>
                    <label className="text-sm font-black text-naranja/80 mb-1 block">RUC *</label>
                    <input
                      type="text"
                      value={formRuc}
                  ...
                  {/* Sector */}
                  <div>
                    <label className="text-sm font-black text-naranja/80 mb-1 block">Sector *</label>
                    <input
                      type="text"
                      value={formSector}
                  ...
                  {/* Año de fundación */}
                  <div>
                    <label className="text-sm font-black text-naranja/80 mb-1 block">Año de fundación</label>
                    <input
                      type="number"
                      value={formAnio}
                ...
                {/* Descripción */}
                <div>
                  <label className="text-sm font-black text-naranja/80 mb-1 block">Descripción</label>
                  <textarea
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-sm font-black text-naranja/80 mb-1 block">Ubicación</label>
                    <input
                      type="text"
                      value={formUbicacion}
                  ...
                  <div>
                    <label className="text-sm font-black text-naranja/80 mb-1 block">N° de empleados</label>
                    <select
                      value={formEmpleados}
                ...
                {/* Sitio web */}
                <div>
                  <label className="text-sm font-black text-naranja/80 mb-1 block">Sitio web</label>
                  <input
                    type="url"
                    value={formWeb}
                ...
                {/* Beneficios */}
                <div>
                  <label className="text-sm font-black text-naranja/80 mb-2 block">Beneficios destacados</label>
                  <div className="flex flex-wrap gap-2">
                    {BENEFICIOS_OPCIONES.map((b) => (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-2xl text-xs font-black uppercase transition-colors ${
                          formBeneficios.includes(b)
                            ? "bg-azul/10 border border-azul text-azul"
                            : "bg-white border border-gris-oscuro text-[#7e7f88] hover:bg-azul/10 hover:border-azul hover:text-azul"
                        }`}
                      >
                        {b}
                      </button>
                ...
                {/* Logo */}
                <div>
                  <label className="text-sm font-black text-naranja/80 mb-1 block">Logo de la empresa</label>
                  <input
                    type="file"
                    accept="image/jpeg,image/png,image/webp,image/svg+xml"
                    onChange={(e) => setLogoFile(e.target.files[0])}
                    className="w-full text-sm text-slate-500 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-black file:uppercase file:tracking-wider file:bg-[#e5fdf9] file:text-[#20685d] hover:file:bg-[#e5fdf9]/20 file:cursor-pointer"
                  />
                  <div className="mt-2">
                    {logoFile ? (
                      <img
              ...
              {/* 4. FOOTER FIJO AL FINAL (Fuera del área de scroll, pero dentro del form) */}
              <div className="p-6 bg-white border-t border-slate-100 shrink-0">
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-azul hover:bg-azul/80 disabled:opacity-50 text-white py-3 rounded-xl text-sm font-black uppercase tracking-wider transition-colors shadow-md"
                >
                  {loading
                    ? "Guardando..."
                    : editingCompany
