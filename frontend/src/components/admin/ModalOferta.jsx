...
import ModalOfertaPaso1, {
  EmpresaSelector,
} from "./modal-oferta/ModalOfertaPaso1";
      ...
      {steps.map((s, i) => {
        ...
        return (
          <div key={s.n} className="flex items-center gap-6">
            <div
              className={`flex items-center gap-2 ${active ? "text-azul" : done ? "text-[#4CAF50]" : "text-slate-300"}`}
            >
              <div
            ...
            {i < steps.length - 1 && (
              <div
                className={`h-px w-12 ${done ? "bg-[#4CAF50]/80 gap-0" : "bg-slate-200 gap-0"}`}
              />
            )}
          </div>
        );
...
export default function ModalOferta({
  ...
  const empresaActiva = companies.find(
    (c) => String(c.id) === String(form.empresa_id),
  );

  // Validaciones por paso
  const canNext1 =
    form.titulo.trim() && form.ubicacion.trim() && form.empresa_id;
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          ...
          {paso === 2 && <ModalOfertaPaso2 form={form} setForm={setForm} />}
          {paso === 3 && (
            <ModalOfertaPaso3
              preguntas={preguntas}
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t border-slate-100 bg-[#f8fafd] rounded-b-2xl flex items-center justify-between gap-3 shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            {paso < 3 ? (
              <button
                ...
                onClick={() => setPaso((p) => p + 1)}
                disabled={
                  (paso === 1 && !canNext1) || (paso === 2 && !canNext2)
                }
