...
const ESTRELLAS_LABEL = { 1: "Muy malo", 2: "Malo", 3: "Regular", 4: "Bueno", 5: "Excelente" };

// ── Contenido del formulario — igual en modal y drawer ───────
function ContenidoFormulario({ empresa, onCerrar, form, handleChange, errors, enviando, exito, handleEnviar }) {
  if (exito) {
    return (
    ...
    <div className="p-6 flex flex-col gap-5">
      ...
      <div>
        ...
        <select value={form.relacion} onChange={e => handleChange("relacion", e.target.value)}
          ...
          {RELACION_OPTS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
        </select>
        {errors.relacion && <p className="mt-1 text-xs text-red-500">{errors.relacion}</p>}
      </div>
      ...
      <div>
        ...
        <select value={form.tiempo_relacion} onChange={e => handleChange("tiempo_relacion", e.target.value)}
          className="w-full px-4 py-2.5 text-sm text-black/80 rounded-xl border border-turquesa focus:border-turquesa">
          <option value="">Selecciona una opción</option>
          {TIEMPO_OPTS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
        </select>
        {errors.tiempo_relacion && <p className="mt-1 text-xs text-red-500">{errors.tiempo_relacion}</p>}
      </div>
      ...
      <div>
        ...
        <div className="flex items-center gap-4">
          <StarRating value={form.estrellas} onChange={v => handleChange("estrellas", v)} size="lg" />
          {form.estrellas > 0 && (
        ...
        {errors.estrellas && <p className="mt-1 text-xs text-red-500">{errors.estrellas}</p>}
      </div>
      ...
      <div>
        ...
        <textarea value={form.texto_positivo} onChange={e => handleChange("texto_positivo", e.target.value)}
          rows={3} maxLength={500} placeholder="Cuéntanos lo positivo de tu experiencia..."
      ...
      <div>
        ...
        <textarea value={form.texto_negativo} onChange={e => handleChange("texto_negativo", e.target.value)}
          rows={3} maxLength={500} placeholder="Cuéntanos qué se podría mejorar..."
          className="w-full px-4 py-2.5 text-sm text-black/80 rounded-xl border border-turquesa focus:outline-none focus:border-turquesa resize-none" />
      </div>
      ...
      <div>
        ...
        <div className="flex flex-col gap-2">
          {RECOMENDARIA_OPTS.map(o => (
            <button key={o.value} type="button" onClick={() => handleChange("recomendaria", o.value)}
        ...
        {errors.recomendaria && <p className="mt-1 text-xs text-red-500">{errors.recomendaria}</p>}
      </div>
...
export default function FormularioEvaluacion({
  ...
  const props = { empresa, onCerrar, form, handleChange, errors, enviando, exito, handleEnviar };
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-between p-6 border-b border-[#e8edf5]">
            <div>
              <h2 className="text-lg font-bold text-[#1c2a52] font-heading">Evaluar empresa</h2>
              <p className="text-sm text-[#6b7a9f] mt-0.5">{empresa?.nombre}</p>
            </div>
            <button onClick={onCerrar} className="p-2 rounded-xl hover:bg-[#f4f6fb] text-[#6b7a9f] transition-colors">
    ...
    <>
      ...
      <div className="fixed top-0 right-0 z-50 h-full w-full max-w-md bg-white shadow-2xl flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#e8edf5] shrink-0">
          <div>
            <h2 className="text-base font-bold text-[#1c2a52] font-heading">Calificar empresa</h2>
